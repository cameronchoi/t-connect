...
THREE.ThreeMFLoader.prototype = {
  ...
  parse: function (data) {
    ...
    function loadDocument (data) {
      ...
      try {
        ...
        if (e instanceof ReferenceError) {
          console.error(
            'THREE.ThreeMFLoader: jszip missing and file is compressed.'
          )
          return null
        }
      }
      ...
      for (var i = 0; i < modelPartNames.length; i++) {
        ...
        if (TextDecoder === undefined) {
          console.error(
            'THREE.ThreeMFLoader: TextDecoder not present. Please use a TextDecoder polyfill.'
          )
          return null
        }

        let fileText = new TextDecoder('utf-8').decode(view)
        let xmlData = new DOMParser().parseFromString(
          fileText,
          'application/xml'
        )
        ...
        if (xmlData.documentElement.nodeName.toLowerCase() !== 'model') {
          console.error(
            'THREE.ThreeMFLoader: Error loading 3MF - no 3MF document found: ',
            modelPart
          )
        }
    ...
    function parseRelsXml (relsFileText) {
      let relsXmlData = new DOMParser().parseFromString(
        relsFileText,
        'application/xml'
      )
    ...
    function parseResourcesNode (resourcesNode) {
      ...
      if (basematerialsNode) {
        resourcesData['basematerial'] = parseBasematerialsNode(
          basematerialsNode
        )
      }
